...
# Agent, A2A(Agent - to - Agent) 아키텍처
- 개요
    - 어떤 작업은 하나의 거대한 모델(llm)이 모두 처리하는 것이 아님
    - 전문환된 페르소나를 가진 에이전트들이 서로 결과물을 입력받아서 작업을 고도화하는 방식(협업)
        - User Task : 사용자가 주제를 던짐
        - Agent A (Developer) : 요청을 받아서 1차 코드 작성
        - Agent B (Reviewer)  : 앞에서 작성된 코드를 받아서 분석, 보안 취약점 등등 체크 리뷰
        (지적)
